...
class Upload
{
	...
	private function setDriver($driver = null, $config = null)
	{
		...
		$config = $config ? : ($this->driverConfig ? : null);
		$class = strpos($driver, '\\') ?
				$driver : 'system\\core\\'
	...
	private function error($errorNo)
	{
		switch ($errorNo) {
			case 1:
				$this->error = '上传的文件超过了 php.ini 中 '
						. 'upload_max_filesize 选项限制的值！';
				break;
			case 2:
				$this->error = '上传文件的大小超过了 HTML 表单中'
	...
	{
		return empty($this->config['mimes']) ? true :
				in_array(strtolower($mime), $this->mimes);
	}
	...
	{
		return empty($this->config['exts']) ? true : in_array(strtolower($ext),
						$this->exts);
	}
	...
	private function getSubPath($filename)
	{
		...
		if ($this->autoSub && !empty($rule)) {
			...
			if (!empty($subpath) && !$this->uploader
							->mkdir($this->savePath . $subpath)) {
				$this->error = $this->uploader->getError();
				return false;
			}
		}
